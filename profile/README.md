# MANARA Lab — UM6P College of Computing
**Multimodal AI, Networks, Analytics, Recommender Systems & Augmented Sensing Laboratory**  
University Mohammed VI Polytechnic (UM6P) · College of Computing · Benguerir, Morocco

---

## 🔭 About the Lab
MANARA Lab investigates the design and evaluation of **trustworthy, intelligent, and networked
systems** at the intersection of AI, cybersecurity, speech processing, and healthcare. Our
research spans algorithmic foundations and real-world deployable systems, bridging AI methods
with networked and physical system constraints. We place a strong emphasis on open and
reproducible science.

**Principal Investigator:** [Prof. Youssef Iraqi](https://scholar.google.com/citations?user=0LdsuWgAAAAJ&hl=en)  
Senior Member IEEE · ACM Member · Associate Professor, College of Computing, UM6P

---

## 🧭 Research Axes

| Axis | Topics |
|------|--------|
| 🤖 Trustworthy & Explainable AI | Explainable AI (XAI), trust models, fairness, cross-domain and blockchain-based recommender systems, authorship verification |
| 📡 Wireless Networks & 6G | NOMA, IRS, UAV communications, HARQ, LEO satellite, IoT backhauling |
| 🔐 Security, Privacy & Blockchain | Physical layer security, cloud forensics, decentralized trust, federated learning with privacy guarantees, smart contracts |
| 🎙️ Speech & Affective Computing | Speaker identification, speech emotion recognition, overlapping speech detection, multimodal audio-visual processing |
| 🏥 Healthcare AI & Medical Imaging | Cardiovascular risk prediction, medical image analysis, AI-driven clinical decision support, student dropout detection |
| 📱 Mobile Crowdsensing | Task allocation, location privacy, crowdsensing frameworks |

---

## 📦 Repositories
Our repos are organized by research axis. Each publication-linked repo includes:
- 📄 Paper reference and DOI
- 🧪 Reproducible experiments
- 📊 Datasets or links to data
- 📜 Open-source license

📚 **[Full Publication List](https://github.com/MANARA-Lab-UM6P/publications)** — all MANARA
Lab journal articles, conference papers, and patents (2021–present).

---

## 👨‍🎓 Team

**Principal Investigator:** [Prof. Youssef Iraqi](https://scholar.google.com/citations?user=0LdsuWgAAAAJ&hl=en)  
Senior Member IEEE · ACM Member · Associate Professor, College of Computing, UM6P

**Associated Faculty:** Prof. Loubna Mekouar  
Assistant Professor, College of Computing, UM6P · Trust management, recommender systems,
federated learning, and mobile crowdsensing

**PhD Students:** Abderrafi Abdeddine · Yassin Terraf · Imane Akdim · Abdelghafour Aboukacem ·
Yasser El-Jarida · Ayoub El-Khouzari · Youness Soussou · Charaf Taii · Nawfal Abbassi

---

## 🎓 Alumni

| Name | Role | Period | Current Position |
|------|------|--------|-----------------|
| Mourad Bahani | Postdoctoral Researcher — AI for Healthcare | 2025–2026 | Assistant Professor, Faculty of Sciences, Mohammed V University, Rabat, Morocco |
| Abla Bedoui | Postdoctoral Researcher — Resource Management in NOMA Networks | 2023 | Research Scientist, Long Island University, New York, USA |

---

## 🤝 Collaborations & Funding
- 🏭 OCP Group — SmartAI-Sort: AI-Based Real-Time Prediction and Adaptive Sorting
- 🇬🇧 British Council — Strengthening Climate Resilience in Morocco (with Cardiff Met)
- 🇩🇪 Erasmus+ KA171 — Mobility Program with Ruhr University Bochum
- 🇪🇸 UPC-UM6P — LCARS: Leveraging Clustering to Advance Recommender Systems
- 🏴󠁧󠁢󠁷󠁬󠁳󠁥 Wales Innovation Network — IntelliCAV: Intelligent Security for Connected and
Autonomous Vehicles

---

## 📬 Contact
📧 Youssef.Iraqi@um6p.ma  
🌐 [College of Computing — UM6P](https://cc.um6p.ma)  
💼 [MANARA Lab on LinkedIn](https://www.linkedin.com/company/manara-lab-um6p)  
📍 Benguerir, Morocco
